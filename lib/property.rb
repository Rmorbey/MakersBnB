...
class Property
  ...
  def initialize(id:, description:, contact:, picture_url:, price_per_night:)
    @id = id
  ...
  def self.add(description:, contact:, picture_url:, user_id:, price_per_night:)
    ...
    Property.new(id: result[0]['id'], description: result[0]['description'], contact: result[0]['contact'], picture_url: result[0]['picture_url'], price_per_night: result[0]['price_per_night'])
  end

  def self.view_all
    result = DatabaseConnection.query('SELECT * FROM properties;')
    result.map do |property| 
      Property.new(id: property['id'], description: property['description'], contact: property['contact'], picture_url: property['picture_url'], price_per_night: result[0]['price_per_night'])
    end
  end
